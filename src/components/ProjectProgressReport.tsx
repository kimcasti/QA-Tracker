...
export default function ProjectProgressReport({ projectId }: ProjectProgressReportProps) {
  ...
  const stats = useMemo(() => {
    ...
    const passedTests = Array.from(lastExecutions.values()).filter(ex => ex.result === TestResult.PASSED).length;
    const failedTests = Array.from(lastExecutions.values()).filter(ex => ex.result === TestResult.FAILED).length;
    
    const bugsCount = [...regressionCycles, ...smokeCycles].reduce((acc, c) => 
      acc + (c.executions || []).filter(ex => ex.bugId).length, 0
    );
  ...
  const statusData = [
    ...
    { name: 'Otras', value: stats.totalFuncs - stats.completedFuncs - stats.testingFuncs, color: '#cbd5e1' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <Title level={2} className="!mb-1">Reporte de Progreso</Title>
          <Paragraph type="secondary">Resumen ejecutivo del estado de calidad y avance del proyecto.</Paragraph>
        </div>
        <Space>
          <Button icon={<FileWordOutlined />} className="rounded-xl h-11 border-slate-200">Exportar Word</Button>
          <Button 
            type="primary" 
      ...
      <Row gutter={20}>
        <Col span={6}>
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <Statistic 
              title={<Text type="secondary" className="text-[11px] font-bold uppercase tracking-wider">Progreso Funcional</Text>}
              value={stats.funcProgress}
              suffix="%"
              prefix={<DashboardOutlined className="text-blue-500 mr-2" />}
              valueStyle={{ color: '#1e293b', fontWeight: 800 }}
            />
            <Progress percent={stats.funcProgress} size="small" strokeColor="#3b82f6" className="mt-2" />
          </Card>
        </Col>
        <Col span={6}>
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <Statistic 
              title={<Text type="secondary" className="text-[11px] font-bold uppercase tracking-wider">QA Coverage</Text>}
              value={stats.testProgress}
              suffix="%"
              prefix={<SafetyCertificateOutlined className="text-emerald-500 mr-2" />}
              valueStyle={{ color: '#1e293b', fontWeight: 800 }}
            />
            <Progress percent={stats.testProgress} size="small" strokeColor="#10b981" className="mt-2" />
          </Card>
        </Col>
        <Col span={6}>
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <Statistic 
              title={<Text type="secondary" className="text-[11px] font-bold uppercase tracking-wider">Tasa de Éxito</Text>}
              value={stats.passRate}
              suffix="%"
              prefix={<CheckCircleOutlined className="text-emerald-500 mr-2" />}
              valueStyle={{ color: '#1e293b', fontWeight: 800 }}
            />
            <Text type="secondary" className="text-xs">{stats.executedTests} ejecutados de {stats.totalTests}</Text>
          </Card>
        </Col>
        <Col span={6}>
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <Statistic 
              title={<Text type="secondary" className="text-[11px] font-bold uppercase tracking-wider">Bugs Totales</Text>}
              value={stats.bugsCount}
              prefix={<BugOutlined className="text-rose-500 mr-2" />}
              valueStyle={{ color: '#e11d48', fontWeight: 800 }}
            />
            <Text type="secondary" className="text-xs">{stats.failedTests} casos fallidos en total</Text>
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={20}>
        <Col span={12}>
          <Card 
            ...
            title={<div className="flex items-center gap-2"><PieChartOutlined className="text-blue-500" /> <span className="font-bold">Estado de Funcionalidades</span></div>}
          >
        ...
        <Col span={12}>
          <Card 
            className="rounded-3xl border-slate-100 shadow-sm h-full"
            title={<div className="flex items-center gap-2"><AlertOutlined className="text-rose-500" /> <span className="font-bold">Indicadores de Riesgo</span></div>}
          >
            <div className="space-y-6 py-4">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-bold text-slate-700">Cobertura de Automatización</span>
                  <span className="text-sm font-bold text-slate-900">
                    {testCases.length > 0 ? Math.round((testCases.filter(tc => tc.testType === TestType.SANITY).length / testCases.length) * 100) : 0}%
                  </span>
                </div>
                <Progress 
                  percent={testCases.length > 0 ? Math.round((testCases.filter(tc => tc.testType === TestType.SANITY).length / testCases.length) * 100) : 0} 
                  strokeColor="#8b5cf6" 
                  size="small" 
                />
              </div>
      ...
      {/* Summary Table */}
      <Card className="rounded-3xl border-slate-100 shadow-sm overflow-hidden" title={<span className="font-bold">Detalle de Cobertura por Módulo</span>}>
        <Table 
          ...
          columns={[
            { title: 'Módulo', dataIndex: 'module', key: 'module', render: (t) => <Text strong>{t}</Text> },
            ...
            { 
              ...
              dataIndex: 'progress', 
              key: 'progress',
              render: (p) => <Progress percent={p} size="small" strokeColor={p === 100 ? '#10b981' : '#3b82f6'} />
            }
          ]}
          pagination={false}
